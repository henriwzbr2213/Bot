...
export class ApiClient {
  ...
  async createApp(ownerDiscordId: string, name: string, region: Region): Promise<AppModel> {
    const res = await fetch(`${API_BASE_URL}/apps`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ownerDiscordId, name, region })
    });
    return res.json();
  }

  async getOrCreate(ownerDiscordId: string, name: string, region: Region): Promise<AppModel> {
    const existing = (await this.listApps(ownerDiscordId)).find((a) => a.name === name);
    return existing ?? this.createApp(ownerDiscordId, name, region);
  }
  ...
  async move(appId: string, region: Region): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/apps/${appId}/move`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ region }) });
    return res.json();
  }
